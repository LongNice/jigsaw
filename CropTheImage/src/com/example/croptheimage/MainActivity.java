...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		int cut_random[] = { R.id.imageView1, R.id.imageView2, R.id.imageView3, R.id.imageView4 };// 所有imageview的ID組
		for (int cut_no = 0; cut_no <= 3; cut_no++) {
			...
			if (cut_no == 0) {
				Bitmap cutBitmap = Bitmap.createBitmap(origialBitmap, 20, 20,
						w, h);// 前兩是原圖起始座標,後兩個是需求截圖的大小
			...
			if (cut_no == 1) {
				Bitmap cutBitmap = Bitmap.createBitmap(origialBitmap, w, 20, w,
						h);// 前兩是原圖起始座標,後兩個是需求截圖的大小
				int random = (int) (Math.random() * cut_random.length);
			...
			if (cut_no == 2) {
				Bitmap cutBitmap = Bitmap.createBitmap(origialBitmap, 20, h, w,
						h);// 前兩是原圖起始座標,後兩個是需求截圖的大小
				int random = (int) (Math.random() * cut_random.length);
				while (cut_random[random] == 1) {
					random = (int) (Math.random() * cut_random.length);
			...
			if (cut_no == 3) {
				Bitmap cutBitmap = Bitmap.createBitmap(origialBitmap, w, h, w,
						h);// 前兩是原圖起始座標,後兩個是需求截圖的大小
				int random = (int) (Math.random() * cut_random.length);
				while (cut_random[random] == 1) {
					random = (int) (Math.random() * cut_random.length);
